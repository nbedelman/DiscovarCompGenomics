...
def LoglikCalc(X,cArr,pArr,lmbd):
	...
	return log(L)
def TemplikCalc(XQList,testC,qIndex,lmbd):
	L=0
	for tup in XQList:
		#MOVED LOG
		L+=log(tup[qIndex+1]*(pdf(tup[0],testC,lmbd))+tup[1]*pdf(tup[0],0,lmbd))#SUPER HACKEY FIX
	#	L+=(tempLogCalc(tup[0],testC
	return L
...
def cSearch(Xdat,XQList,ind,lmbd):
	...
	for j in range(0,len(RESCALEXdat)):
		tempLik.append(TemplikCalc(XQList,RESCALEXdat[j],ind,lmbd))
	#print tempLik
...
#	print XQList
	...
	FULLpArr=[pArr]
	FULLlmbd=[lmbd]
	while steps<200:# and abs(likArray[k]-likArray[k-1])>=thresh*abs(likArray[k]):# and steps<100:
		steps+=1
		XQList=XQListCalc(data,cArr,pArr,lmbd)
		pArr=pUpdate(XQList)
#		print pArr
...
#		k=k+1
		...
		#pArr=pUpdate(XQList)
		oldlmbd=lmbd
#BAD		lmbd=monoLUpdate(data,XQList,cArr,pArr,lmbd)
#BAD		lmbd=badSearch(data,XQList,cArr,pArr,lmbd)
##		lmbd=mean(data) #REALLY DUMB TO TEST
		...
		if vect and ind>=0:
			for x in range(1,4):
				...
				if vect[x]:
					...
					cArr,pArr,MLEst,BIC=InitEM(scaled,numDist,0.05,0,"FULL")
				else:
					cArr,pArr,MLEst,BIC=[],[],[],[]
