...
def cSearch(double[:] Xdat,list XQList,int ind,double lmbd,list cArr):
...
def cFinder(double[:] Xdat,list XQList,list cArr,double lmbd):
...
def gradAscent(np.ndarray Xdat,list XQList,list cArr,list pArr,double lmbd, double SCALAR):
	if len(cArr)==1:
		...
		stepScale=SCALAR
		h=1
		while abs(likArray[k]-likArray[k-1])>=0.001*stepScale*abs(likArray[k]) and k<50 and h<50:
