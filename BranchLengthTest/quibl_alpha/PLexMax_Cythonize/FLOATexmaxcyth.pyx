...
def pdf(float x,float C,float lmbd):
	cdef float y
	if x>0 and x>=C*lmbd:
...
def TemplikCalc(list XQList, float testC, int CIndex, float lmbd, list cArr):#, dict lookup):
	cdef float L=0
	cdef float temp=0
	cdef float y=0
	cdef float x=0
...
def cLookup(list XQList, list cArr, float lmbd):
	cdef dict lookup={}
	for ind,tup in enumerate(XQList):
		for indi,pival in enumerate(tup[1:]):
...
def cSearch(double[:] Xdat,list XQList,int ind,float lmbd,list cArr):
...
def cFinder(double[:] Xdat,list XQList,list cArr,float lmbd):
	cdef int clen=len(cArr)
	cdef list nuC=[0]
	#print clen
	for i in range(1,clen):
		nuC.append(cSearch(Xdat,XQList,i,lmbd,cArr))
	return nuC
def qCalc(float x, list cArr,list pArr,float lmbd):
	y=[x]
	temp=[]
	...
	y.extend(temp[:])
	return y
def XQListCalc(double[:] Xdat,list cArr,list pArr,float lmbd):
	XQList=[]
	for x in Xdat:
		XQList.append(qCalc(x,cArr,pArr,lmbd))
	return XQList
def LoglikCalc(float X,list cArr,list pArr,float lmbd):
	cdef double L
	L=0
...
def fullLikCalc(list XQList,double[:] Xdat,list cArr,list pArr,float lmbd):
	tempSum=0
	for X in Xdat:
		tempSum+=(LoglikCalc(X,cArr,pArr,lmbd))
	return tempSum

def calcDeriv(list XQList,list cArr,list pArr,float lmbd):
	cdef float val
	cdef float alpha
	cdef float beta
	deriv=0
...
def gradAscent(np.ndarray Xdat,list XQList,list cArr,list pArr,float lmbd, float SCALAR):
	if len(cArr)==1:
		...
		stepScale=SCALAR
		h=1
		while abs(likArray[k]-likArray[k-1])>=0.01*abs(likArray[k]) and k<50 and h<50:
