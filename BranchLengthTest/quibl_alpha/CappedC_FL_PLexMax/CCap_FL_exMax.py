...
def InitEM(datafp,numDist,thresh,numSteps,lmbd,threshh):#, numFlag=None,data_flag=None):
	#print numDist
...
#	print cArr
	...
	while steps<numSteps:# and abs(likArray[k]-likArray[k-1])>=thresh*abs(likArray[k]):# and steps<100:
		...
		cArr=emc.cFinder(data,XQList,cArr,lmbd,threshh)
#		oldlmbd=lmbd
...
def PLDataAuto(datapath,outpath,numDist,numSteps,lmbd,thresh):
	data=pickle.load(open(datapath,"rb"))
	data=asarray(data)
	f=open(outpath,'w')
	num_cores=multiprocessing.cpu_count()
	output=Parallel(n_jobs=num_cores)(delayed(toPar)(ind,vect,numDist,numSteps,lmbd,thresh) for ind,vect in enumerate(data))
	f.write(str(output))

def GuideTreeDataAuto(datapath,outpath,numDist,numSteps,lmbd, guideTree):
	data=pickle.load(open(datapath,"rb"))
	data=asarray(data)
	f=open(outpath,'w')
	num_cores=multiprocessing.cpu_count()
	#output=Parallel(n_jobs=num_cores)(delayed(toPar)(ind,vect,numDist,numSteps,lmbd,thresh) for ind,vect in enumerate(data))
	for ind,vect in enumerate(data):
		toPar(ind,vect,numDist,numSteps,lmbd,thresh)
	f.write(str(output))
...
def toPar(ind,vect,numDist,numSteps,lmbd,thresh):
	out=[]
	fullout=[]
	if vect.size!=0 and ind>=0:
		for x in range(1,4):
			...
			if remZero.size>=10: #vect[x]:
				#scaled=[(y)*float(scaling) for y in vect[x] if y>0]
				cArr,pArr,MLEst,BIC,lmbdEst,maxIndex=InitEM(remZero,numDist,0.1,numSteps,lmbd,thresh)
			else:
...
#pr=cProfile.Profile()
#pr.enable()
PLDataAuto(sys.argv[1],sys.argv[2],int(sys.argv[3]),int(sys.argv[4]),float(sys.argv[5]),float(sys.argv[6]))
#pr.disable()
#s=StringIO.StringIO()
#sortby='ncalls'
