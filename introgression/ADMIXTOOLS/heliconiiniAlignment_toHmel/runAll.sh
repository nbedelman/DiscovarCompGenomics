...
################ Define Variables ###############
refGenome=Heliconius_melpomene_melpomene_hmel2
chromMap=data/mapFile.tsv

################# Run Code ######################
#first, extract MAF for each chromosome
chroms=$(grep ">" data/refGenome.fa |cut -d ">" -f 2|cut -d " " -f 1)
for chrom in $chroms
...
#
# qpDstat -p parFiles/parDstat_eratoClade > eratoClade.D.out
# qpDstat -p parFiles/parDstat_melpomeneClade > melpomeneClade.D.out
