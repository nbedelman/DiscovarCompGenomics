...
setwd("~/Dropbox/HeliconiusComparativeGenomics/")
...
############ Fd ##########
# Fd=read.csv(FD)
# Fd$scaffold <- as.character(Fd$scaffold)
# Fd$chrom <- as.character(unlist(lapply(Fd$scaffold,getChrom)))
# Fd$end <- as.numeric(unlist(lapply(Fd$scaffold,getEnd)))*5000
# Fd$start <- Fd$end-4999
# Fd$start <- as.character(Fd$start)
# Fd$end <- as.character(Fd$end)
# Fd$fd=as.numeric(as.character(Fd$fd))
# FdGranges <- makeGRangesFromDataFrame(Fd,keep.extra.columns = TRUE)
# FdGranges <- sortSeqlevels(FdGranges)
# FdGranges <- sort(FdGranges)
...
kp <- plotKaryotype(genome=HeratoGenomeGrange,plot.type=1,cex=2,plot.params = pp)#, chromosome = c("chr15"))
...
##### plot Fd
# kpAxis(kp,numticks=11, ymin=-0.5,ymax=0.5,r0=0.75, r1=1.5)
# kpPoints(kp, data=FdGranges,y=FdGranges$fd,r0=0.75, r1=1.5,ymin=-0.5,ymax=0.5, border=NA)
