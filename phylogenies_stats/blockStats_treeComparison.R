...
########### Define Data Files #############
STATS="datasets/heliconiini_HeraRef_noBS_fullAlignSites.txt"
TREES="datasets/heliconiini_10KBAbutting_HeraRef_trees.tsv"
RECRATE="/Users/nbedelman/Dropbox/heliconius/MANUSCRIPT/Sections/recombinationRate/HeratoWindows.10KBAbutting.recRates.CDSoverlaps.bed"
out="heliconiini_10KBAbutting_HeraRef"
...
############## Read in data #########
#trees
trees <- read.delim(TREES, header=FALSE, col.names = c("block_id","tree"), sep=" ")
trees <- read.csv(TREES, header=FALSE, col.names = c("block_id","tree"))
trees$block_id <- as.character(trees$block_id)


trees$chrom <- as.character(unlist(lapply(trees$block_id,getChrom)))

#recombRate
rateFile=read.delim(RECRATE, col.names=c("scaffold","start","end","block_id","dot","strand","loess2","loess3","loess4","LD","CDS"))
rateFile$LD <- as.numeric(as.character(rateFile$LD))
...
#10KB windows
stats$end <- as.numeric(unlist(lapply(stats$block_id,getEnd)))*10000
stats$start <- stats$end-9999

#50KB Sliding windows
stats$end <- as.numeric(unlist(lapply(stats$block_id,getEnd)))*25000
stats$start <- stats$end-24999


############### Tidy Data and make overview graphs #########
allData <- inner_join(trees,stats) 
allData <- inner_join(allData, rateFile, by="block_id")
#allData <- stats
allData <- allData %>% drop_na
allData$start <- as.character(allData$start.x)
allData$end <- as.character(allData$end.x)
allData$phi_p <- as.numeric(as.character(allData$phi_p.x))
allData$transformedP <- -log10(allData$phi_p.x)
allData$rateQuint <- as.factor(as.integer(cut(allData$LD, quantile(allData$LD, probs= seq(0,1,.2), na.rm=T), include.lowest=TRUE)))
#allData<- subset(allData, fullLength>=2000)
allData <- subset(allData, mean_bootstrap_support >=80 & fullLength.x>=2000 & fullAlignedSites >1000)
